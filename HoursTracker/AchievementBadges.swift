...
// MARK: - Achievement badge model + factory (Phase 8 extraction)
//
// Moved verbatim from AchievementsView.swift so the view file stays under
// 500 lines. Types renamed Stats -> BadgeStats and Badge -> AchievementBadge
// (now internal so AchievementsSections.swift can render them); all badge
// definitions, thresholds, and progress math are UNCHANGED.
...
struct BadgeStats {
    ...
    init(from store: HoursStore) {
        ...
        bestStreak = BadgeStats.computeBestStreak(dates: Array(workDates), calendar: cal)
        bestLongShift12hStreak = BadgeStats.computeBestLongShiftStreak(entries: entries, calendar: cal, minHours: 12)
        maxMonthlyHours = BadgeStats.computeMaxMonthlyHours(entries: entries, calendar: cal)
        maxMonthlyOvertimeHours = BadgeStats.computeMaxMonthlyOvertimeHours(entries: entries, breakdowns: breakdowns, calendar: cal)
        maxWeeklyHours = BadgeStats.computeMaxWeeklyHours(entries: entries, calendar: cal)
        maxYearlyHours = BadgeStats.computeMaxYearlyHours(entries: entries, calendar: cal)
        monthsWorked = Set(entries.map { cal.date(from: cal.dateComponents([.year, .month], from: $0.date)) ?? $0.date }).count
        hasPerfectMonth = BadgeStats.computeHasPerfectMonth(dates: Array(workDates), calendar: cal)
        hasEveryWeekendWorkedMonth = BadgeStats.computeHasEveryWeekendWorkedMonth(dates: Array(workDates), calendar: cal)
        perfectWeekExists = BadgeStats.computePerfectWeekExists(dates: Array(workDates), calendar: cal)
    }
...
enum BadgeFactory {
    static func makeBadges(stats: BadgeStats) -> [AchievementBadge] {
        func hoursBadge(_ target: Double, icon: String, name: String, detail: String? = nil, order: Int, legend: Bool = false) -> AchievementBadge {
        ...
        func countBadge(_ value: Int, target: Int, icon: String, name: String, detail: String, order: Int, legend: Bool = false) -> AchievementBadge {
        ...
        func doubleBadge(_ value: Double, target: Double, icon: String, name: String, detail: String, order: Int, legend: Bool = false) -> AchievementBadge {
        ...
        func boolBadge(_ value: Bool, icon: String, name: String, detail: String, order: Int, legend: Bool = false) -> AchievementBadge {
            return AchievementBadge(
                icon: icon,
                name: name,
                detail: detail,
        ...
        func appendUnique(_ badge: AchievementBadge) {
